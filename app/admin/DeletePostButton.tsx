...
export default function DeletePostButton({ id, title }: { id: string; title: string }) {
  ...
  async function handleDelete() {
    ...
    let res: Response;
    try {
      res = await fetch(`/api/admin/posts/${id}`, { method: "DELETE" });
    } catch {
      setLoading(false);
  ...
  return (
    <button
      ...
      className="flex min-h-11 items-center gap-1.5 rounded-lg px-2.5 text-sm text-red-600 transition-colors hover:bg-red-50 disabled:opacity-50"
    >
      {loading ? (

...
import { AlertCircle, Check, Eye, EyeOff, KeyRound, Loader2, Lock } from "lucide-react";
...
function ResetPasswordForm() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (password.length < PASSWORD_MIN_LENGTH) {
      setError(`Das Passwort braucht mindestens ${PASSWORD_MIN_LENGTH} Zeichen.`);
      return;
    }
  ...
  const inputClass =
    "w-full rounded-xl border border-black/10 bg-white py-3.5 pl-10.5 pr-11 text-base text-text-primary outline-none transition-colors focus:border-sage focus:ring-2 focus:ring-sage/20";
      ...
      >
        <div className="mb-6 flex flex-col items-center text-center">
          <div className="flex h-14 w-14 items-center justify-center rounded-full bg-sage shadow-[0_8px_24px_-6px_rgba(143,175,138,0.55)] ring-1 ring-sage/30">
            <KeyRound className="h-6 w-6 text-ivory" strokeWidth={1.75} />
          </div>
        </div>

        <div className="w-full rounded-3xl bg-white/70 p-6 shadow-xl shadow-text-primary/10 ring-1 ring-black/5 backdrop-blur-xl sm:p-8">
          <h1 className="font-serif text-2xl text-text-primary">Neues Passwort</h1>

          {!token ? (
            <>
              <p className="mt-1.5 text-sm text-text-secondary">
                Dieser Link ist unvollständig. Bitte fordere über die Anmeldeseite einen neuen an.
              </p>
              <Link
                href="/admin/login"
                className="mt-6 flex w-full items-center justify-center rounded-full bg-sage px-4 py-3.5 text-base font-medium text-ivory ring-1 ring-sage/30"
              >
                Zur Anmeldung
              </Link>
            </>
            ...
            <>
              <p className="mt-4 flex items-start gap-2 rounded-xl bg-sage/10 px-3 py-2.5 text-sm text-text-primary ring-1 ring-sage/25">
                ...
                <span>
                  Passwort gespeichert. Du wirst zur Anmeldung weitergeleitet — melde dich dort mit
                  deiner E-Mail und dem neuen Passwort an.
                </span>
              </p>
              <Link
                href="/admin/login"
            ...
            <form onSubmit={handleSubmit}>
              <p className="mt-1.5 text-sm text-text-secondary">
                Wähle ein neues Passwort für den Admin-Bereich.
              </p>
              ...
              <label className="mt-4 block text-sm font-medium text-text-primary" htmlFor="repeat">
